...
def trsbox(xopt, gopt, hq, sl, su, delta):
    n = xopt.size
    assert gopt.size == n, "gopt and xopt have incompatible sizes"
    assert hq.size == n * (n + 1) // 2, "hq and xopt have incompatible sizes"
    assert sl.size == n, "sl and xopt have incompatible sizes"
    assert su.size == n, "su and xopt have incompatible sizes"
    assert delta > 0.0, "delta must be strictly positive"
    # Assume gopt and hq have full quadratic model for objective
    ...
    for ii in range(MAX_LOOP_ITERS):
        ...
        # Exit condition TODO different from BOBYQA
        if zhang_code_structure:
            if stplen <= 1.0e-30:  # DFBOLS
                need_alt_trust_step = False
                break  # break and quit
        else:
            if False:  # BOBYQA (i.e. no exit condition here)
                need_alt_trust_step = False
                break  # break and quit
        ...
        if zhang_code_structure:
            ...
            if iterc == itermax or sdec <= 1.0e-2 * qred:  # BOBYQA
                need_alt_trust_step = False
                break  # break and quit
    ...
    if need_alt_trust_step:
        crvmin = 0.0
        d, gnew = alt_trust_step(n, xopt, hq, sl, su, d, xbdi, nact, gnew, qred)
...
"""
Alternative trust region step (starts at label 100 of Fortran code, where crvmin = 0)
...
def alt_trust_step(n, xopt, hq, sl, su, d, xbdi, nact, gnew, qred):
    MAX_LOOP_ITERS = 100 * n ** 2  # avoid infinite loops
